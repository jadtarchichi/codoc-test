<template>
  <div class="search-text-input">
    <div v-if="!searchText" class="placeholder-icon">
      <svg
        width="1"
        height="30"
        viewBox="0 0 1 30"
      ...
      >
        <line x1="0.5" x2="0.5" y2="30" stroke="#343A40" />
      </svg>
    </div>
...
<script>
import "@/styles/search-text-input.scss";
...
export default {
  ...
  watch: {
    searchText: function (newText) {
      this.$emit("search-text-change", newText);
    },
  ...
  methods: {
    onSearch() {
      this.$emit("on-search", { text: this.searchText });
    },
  },
};
</script>
